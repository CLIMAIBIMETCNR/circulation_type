...
var Windy = function Windy(params) {
	...
	var VELOCITY_SCALE =0.02; //0.005 * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); 
// scale for wind velocity (completely arbitrary--this value looks nice)
	
   

	var MIN_TEMPERATURE_K = params.mintk; // step size of particle intensity color scale
	var MAX_TEMPERATURE_K = params.maxtk; // wind velocity at which particle intensity is maximum (m/s)
	var MAX_PARTICLE_AGE = params.age; // max number of frames a particle is drawn before regeneration
	var PARTICLE_LINE_WIDTH = params.width; // line width of a drawn particle
...
L.Control.WindPosition = L.Control.extend({
	...
	_onMouseMove: function _onMouseMove(e) {
		...
		if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
			...
			htmlOut = "<strong>Wind Direction: </strong>" + self.vectorToDegrees(gridValue[0], vMs) + "°" + ", <strong>Wind Speed: </strong>" + self.vectorToSpeed(gridValue[0], vMs).toFixed(1) + "m/s" + ", <strong>Temp: </strong>" + (gridValue[2]).toFixed(1) + "°C";
		} else {
...
(function (root, factory) {
	...
	var WindJSLeaflet = {
		...
		init: function init(options) {
			...
			WindJSLeaflet._checkWind(options).then(function () {
                ...
                WindJSLeaflet._source = options.source;
                WindJSLeaflet._argMode = options.argMode;
                WindJSLeaflet._sourcedata = options.sourcedata;
                

                WindJSLeaflet._age = parseInt(options.age);
		        WindJSLeaflet._width = parseInt(options.width);
		        WindJSLeaflet._mintk = parseFloat(options.mintk);
		        WindJSLeaflet._maxtk = parseFloat(options.maxtk);
                WindJSLeaflet._caption = options.caption;

				// create canvas, add overlay control

				WindJSLeaflet._canvasLayer = L.canvasLayer().delegate(WindJSLeaflet);
				WindJSLeaflet._options.layerControl.addOverlay(WindJSLeaflet._canvasLayer, options.caption);
   ...
   */
		...
		_loadargData: function _loadargData() {

			 console.log('using arg data..');

             WindJSLeaflet._data = WindJSLeaflet._sourcedata;
             WindJSLeaflet._initWindy(WindJSLeaflet._sourcedata);
		...
		_loadLocalData: function _loadLocalData() {
			...
			$.getJSON(WindJSLeaflet._source, function (data) {
		...
		_initWindy: function _initWindy(data) {
			...
			// windy object
			this._windy = new Windy({ canvas: WindJSLeaflet._canvasLayer._canvas, 
				                      data: data,
				                      mintk:WindJSLeaflet._mintk,
				                      maxtk:WindJSLeaflet._maxtk,
				                      age:WindJSLeaflet._age,
				                      width:WindJSLeaflet._width
				                      });
